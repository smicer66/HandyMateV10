...
export class HomePage {
	...
	constructor(public app: App, public events2: Events, public modalCtrl: ModalController, public platform: Platform, public loadingCtrl: LoadingController, 
		public nativePageTransitions: NativePageTransitions, public http: HttpClient, public storage: Storage, public actionSheetCtrl: ActionSheetController, public toastCtrl: ToastController, public fb: FormBuilder, public navCtrl: NavController, public navParams: NavParams) {
	...
	ionViewDidEnter()
	{
		...
		this.storage.get('handy_mate_loggedInUser').then((val2) => {
			...
			this.storage.get('handy_mate_skills').then((val21) => {
				this.skillsList = val21;
				console.log(val21);
			});
		});
	}
				...
				this.http.post<ProjectListRespInt>("https://handymateservices.com/api/projects-list", parameter, httpOptions).subscribe(
					res1 => {
						...
						if(res1.success==422)
						{
							...
							if(res1.advertYes == 1)
							{
								const advertModal = this.modalCtrl.create(NewContextMenuPage, { advertImage: res1.advertImage, advertUrl: res1.advertUrl });
								advertModal.present();
	...
	viewAllProjects()
	{
		...
		this.navCtrl.push(ProjectsPage, {fromWhere: 0});
	}
	
	
	viewProjects(skill_name, skill_id)
	{
		...
		this.nativePageTransitions.flip(options);
		this.navCtrl.push(ProjectsPage, {fromWhere: 0, skill_id: skill_id, skill_name: skill_name});
	}
	
	
	addNewProjects(skill_name, skill_id)
	{
		...
		this.nativePageTransitions.flip(options);
		this.navCtrl.push(NewProjectStepThreePage, {fromWhere: 0, skill_id: skill_id, skill_name: skill_name});
	}
