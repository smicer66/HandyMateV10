	...
	ionViewDidLoad() {
		...
		this.storage.get('handy_mate_loggedInUser').then((val2) => {
			...
			this.storage.get('handy_mate_token').then((val2) => {
				...
				let url = "https://handymateservices.com/api/get-sub-skills";
				this.http.post<NewProjectReponse>(url, parameter, httpOptions).subscribe(
					res => {
						...
						{
							this.supremeSkillsList = res.subSkills;
							this.skillsList = this.supremeSkillsList;
							console.log('true');
							console.log(this.skillsList);
							console.log(res.artisanSubSkills);
							console.log(res.artisanSubSkills.length);
							
							for(var i5=0; i5<res.artisanSubSkills.length; i5++)
							{
								this.userSkills.push(res.artisanSubSkills[i5].sub_skill_id);
							}
	...
	filterList(selectedValue: any)
	{
		...
		if(this.newProjectDataThree.search.trim().length>0)
		{
			...
			tempSkills = tempSkills.filter(currentSkill => {
				...
				if (currentSkill.sub_skill_name && this.newProjectDataThree.search) {
					var strpos = currentSkill.sub_skill_name.toLowerCase().indexOf(this.newProjectDataThree.search.toLowerCase());
	...
	updateMySkills(newProjectDataThree)
	{
		...
		if(this.checkedItems.length >0)
		{
			...
			this.storage.get('handy_mate_token').then((val2) => {
				...
				let parameter = form_params;
				let url = "https://handymateservices.com/api/post-user-sub-skills";
				this.http.post<NewProjectReponse>(url, parameter, httpOptions).subscribe(
					res => {
						this.loading.dismiss();
						let success: any = null;
						...
						{
							this.storage.set('handy_mate_artisan_sub_skills', (res.artisanSubSkills)).then((xx) => {
