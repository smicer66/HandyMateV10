...
@IonicPage()
@Component({
  selector: 'page-upload-attachment',
  templateUrl: 'upload-attachment.html',
})
export class UploadAttachmentPage {
	...
	constructor(public app: App, public camera: Camera, public platform: Platform, public storage: Storage, public loadingCtrl: LoadingController, public http: HttpClient, 
	...
	selectImageFromGallery(type)		//type==0 ? SELECT : 1 - Take Photo
	{
		...
		this.camera.getPicture(options).then((imageData) => {
			...
			let base64ImageFromGallery = 'data:image/jpeg;base64,' + imageData;
			this.viewCtrl.dismiss({base64ImageFromGallery: base64ImageFromGallery, id: this.id});
		}, (err) => {
			// Handle error
			console.log(err);
		});
