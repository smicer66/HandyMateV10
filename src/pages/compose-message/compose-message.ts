...
interface MessageRespInterface{
	project: any;
	success: any;
	message: any;
}
...
export class ComposeMessagePage {
	...
	msgDataFormData = { msg_details: ''};
	msgDataForm : FormGroup;
	msg_details: AbstractControl;
	token: any;
	loading: any;
	user: any;
	bid: any = null;
	receipient_first_name:any;
	receipient_last_name:any;
	message: any;
	messageBidder: any;
		...
		public nativePageTransitions: NativePageTransitions)
	{
		...
		this.msgDataForm = this.fb.group({
			'msg_details': [null, Validators.compose([Validators.required])]
		});


		this.msg_details = this.msgDataForm.controls['msg_details'];
		this.projectDetails = navParams.get('project');
		this.message = navParams.get('message');
		this.receipient_first_name = navParams.get('receipient_first_name');
		this.receipient_last_name = navParams.get('receipient_last_name');
		this.user = navParams.get('user');
		this.messageBidder = navParams.get('bid');
		console.log(this.user);
		console.log(this.projectDetails);
		console.log(this.messageBidder);
	}
	
	submitProjectMessage(project, message, user, bid)
	{
			console.log(project);
			console.log(message);
			...
			this.storage.get('handy_mate_token').then((val) => 
			{
				...
				url = "https://handymateservices.com/api/compose-project-message";
				form_params = form_params + "message=" + encodeURI(this.msgDataFormData.msg_details);
				if(message!=undefined && message!=null)
				{
					form_params = form_params + "&msgId=" + message.id;
				}
				if(project!=undefined && project!=null)
				{
					form_params = form_params + "&projectId=" + projectId;
				}
				if(user.role_type=='Artisan')
				{
					form_params = form_params + "&receiver=" + project.created_by_user_id;
				}
				if(user.role_type=='Private Client' || user.role_type=='Corporate Client')
				{
					if(message!=undefined && message!=null)
					{
						form_params = form_params + "&receiver=" + message.sender_user_id;
					}
					else
					{
						if(bid!=undefined && bid!=null)
							form_params = form_params + "&receiver=" + bid.bid_by_user_id;
					}
				}
				...
				this.http.post<MessageRespInterface>(url, parameter, httpOptions).subscribe(
					res1 => {
						console.log(res1);
