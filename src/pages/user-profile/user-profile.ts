...
export class UserProfilePage {
	...
	portfolio: any = [];
	userPortolioForm : FormGroup;
	
	
	constructor(public app: App, 
		public nativePageTransitions: NativePageTransitions, public platform: Platform, public storage: Storage, public modalCtrl: ModalController, 
		public loadingCtrl: LoadingController, public http: HttpClient, public toastCtrl: ToastController, public fb: FormBuilder, public viewCtrl: ViewController, public navCtrl: NavController, public navParams: NavParams) {
		console.log(navParams);
		this.userSelected = this.navParams.get('user');
		this.userPortolioForm = this.fb.group({
	...
	ionViewDidEnter() {
		this.storage.get('handy_mate_loggedInUser').then((val2) => {
			...
			this.getUser(this.userSelected);
		});
	}
	...
	getUser(user)
	{
		...
		this.storage.get('handy_mate_token').then((val2) => {
			...
			let url = "https://handymateservices.com/api/get-user-profile-details";
			this.http.post<UserDataRespInt>(url, parameter, httpOptions).subscribe(
				res => {
					...
					{
						this.userProfileSelected = res.userProfile;
						this.projects_completed = res.projects_completed;
						this.amount_spent = res.amount_spent;
						this.projects_count = res.projects_count;
						
						var x3_ = [];
						for(let x3=0; x3< res.portfolios.length; x3++)
						{
							x3_.push({file_name: 'http://handymateservices.com/img/clients/' + res.portfolios[x3].file_name});
						}
						this.portfolio = x3_;
					}
					else
	...
	uploadPortfolios()
	{
		this.storage.get('handy_mate_loggedInUser').then((val2) => 
		{
			...
			this.storage.get('handy_mate_token').then((val2) => {
				...
				let parameter = form_params;
				let url = "https://handymateservices.com/api/upload-portfolio";
				this.http.post<UploadPortfolioReponse>(url, parameter, httpOptions).subscribe(
					res => {
						this.loading.dismiss();
						let success: any = null;
						success = res.success;
						...
						{
							this.portfolio = res.portfolios;
							this.attach_image_url1 = 'assets/imgs/attach_image.png';
							this.attach_image_url2 = 'assets/imgs/attach_image.png';
							this.attach_image_url3 = 'assets/imgs/attach_image.png';
...
interface UserDataRespInt{
	...
	first_name: any;
	last_name: any;
	current_image_url: any;
	total_user_rating: any;
	rating_count: any;
	country: any;
	province: any;
	projects_count: any;
	profile: any;
	projectReviewList: any;
	success: any;
	message: any;
	projects_completed: any;
	amount_spent: any;
...
interface UploadPortfolioReponse{
	success: any;
	message: any;
	portfolios: any;
}
