...
export class SupportTicketsPage {
		...
		public toastCtrl: ToastController, public fb: FormBuilder, public navCtrl: NavController, public navParams: NavParams)
	{
		...
		let messageThreads = navParams.get('messageThreads');
		let messageThreadDetails = navParams.get('messageThreadDetails');
		
		if(messageThreads!=undefined && messageThreads!=null && messageThreadDetails!=undefined && messageThreadDetails!=null)
		{
	...
	ionViewDidEnter() {
		console.log('ionViewDidLoad LoansPage');
		let messageThreads = this.navParams.get('messageThreads');
		let messageThreadDetails = this.navParams.get('messageThreadDetails');
	...
	showMessage(messageThread, user)
	{
		...
		this.navCtrl.push(ViewTicketPage, {messageThread: messageThread, user:user});
	}
	...
	refreshMessageList()
	{
		this.storage.get('handy_mate_loggedInUser').then((val2) => {
			...
			this.storage.get('handy_mate_token').then((val2) => {
				...
				let url = "https://handymateservices.com/api/support-message-thread-list";
	...
	openTicket()
	{
		...
		this.nativePageTransitions.flip(options);
		this.navCtrl.push(RaiseSupportTicketPage);
	}
